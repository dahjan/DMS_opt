...
def data_split(Ag_pos, Ag_neg):
    ...
    Test_AgNeg_amt = len(Ag_combined.iloc[idx_test2, :]['AgClass']) - Ag_combined.iloc[idx_test, :].iloc[idx_test2,:]['AgClass'].sum()
    Adj_test_amt = int(Test_AgNeg_amt/0.90)
    Test_AgPos_amt = Adj_test_amt - Test_AgNeg_amt
    
    Test_AgPos = Ag_combined.iloc[idx_test, :].iloc[idx_test2,:].iloc[np.where(Ag_combined.iloc[idx_test, :].iloc[idx_test2,:]['AgClass'] == 1)[0], :]
    Test_AgNeg = Ag_combined.iloc[idx_test, :].iloc[idx_test2,:].iloc[np.where(Ag_combined.iloc[idx_test, :].iloc[idx_test2,:]['AgClass'] == 0)[0], :]
...
def data_split_adj(Ag_pos, Ag_neg, ratio):
    ...
    if len(Ag_pos) <= len(Ag_neg):
        ...
        if len(Ag_neg)/(1-ratio) < data_size:
            ...
            Ag_pos = Ag_pos[0:int((new_data_size*ratio))]
            Ag_neg = Ag_neg
            
        if len(Ag_neg)/(1-ratio) >= data_size:
        
            Ag_pos = Ag_pos[0:int((data_size*(ratio)))]
            Ag_neg = Ag_neg[0:int((data_size*(1-ratio)))]
    else:
        ...
        if len(Ag_pos)/(ratio) < data_size:
            ...
            Ag_pos = Ag_pos
            Ag_neg = Ag_neg[0:(int(new_data_size*(1-ratio)))]
