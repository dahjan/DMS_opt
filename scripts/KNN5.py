...
def KNN_classification(dataset, ratio, filename):
    ...
    KNN_classifier = KNeighborsClassifier(n_neighbors = 100, metric = 'minkowski', p = 2)
    ...
    # ROC curve on test2
    y_score2 = KNN_classifier.predict_proba(X_test)
    ...
    plt.title('KNN ROC curve (Train={})'.format(filename))
    plt.legend(loc = 'lower right')
    savefig('figures/KNN_ROC_Test_{}.png'.format(filename))
    plt.cla()
    plt.clf()
    ...
    plt.xlim([0.0, 1.0])
    plt.ylim([0.0, 1.05])
    plt.title('KNN Precision-Recall curve (Train={})'.format(filename))
    plt.legend(loc = 'lower right')
    savefig('figures/KNN_P-R_Test_{}.png'.format(filename))
    plt.cla()
    plt.clf()
